...
class DifferentialDrive:
    def __init__(self) -> None:
        ...
        self.wheel_radius = 0.032 # wheel radius
        self.wheel_base   = 0.235 # wheel base

        # scan matching variables 
        self.min_scan_th_distance = 0.8  # minimum scan taking distance  
        self.min_scan_th_angle = np.pi/4 # take scan angle thershold
        self.max_scan_th_distance = 2  # minimum scan taking distance  
        self.max_scan_th_angle = np.pi/4 # take scan angle thershold
        self.overlapping_check_th_dis = 1 # ovrlapping checking distance thershold 
        self.max_scan_history = 6 # maximum amount of scan history to store 
        self.num_of_scans_to_remove = 2 # number of scans to remove from the history
    ...
    def check_scan(self, scan):
        ...
        if(len(self.map) == 0 and len(self.scan_cartesian)):
        ...
        elif(len(self.scan_cartesian) and check_scan_thershold(self.xk,self.min_scan_th_distance, self.min_scan_th_angle, self.scan_cartesian)):
            self.publish_covariance_marker()
            self.scan_world = scan_to_world(self.scan_cartesian  , self.xk[-3:])
            self.map.append(self.scan_world)
            self.scan.append(self.scan_cartesian)
            ...
            if(len(self.map) > self.max_scan_history):
                ...
                for i in range(self.num_of_scans_to_remove):
                    indices = [i+1 for i in range(len(self.scan))]
                self.scan.pop(indices)
                self.map.pop(indices)
