...
class PoseSLAMEKF:
    ...
    def OverlappingScan(self,xk ,max_num_scans, overlap_distance ):
        ...
        print("xk_pose",xk_pose)
        print("current_pose",current_pose)
        print("matched_pose",matched_pose)

        tree = KDTree(matched_pose)
        # Query the tree for the neareast neighbors
        dist, indices = tree.query(current_pose, k = min( len(matched_pose), k=max_num_scans))
        # Check if the distances are less than the overlap distance
        print("dist",dist , overlap_distance , indices)
