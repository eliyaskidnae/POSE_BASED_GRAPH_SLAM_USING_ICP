...
class PoseSLAMEKF:
    ...
    def F2k(self,xk_robot):
        ...
        Jfw = np.array([[cos(float(xk_robot[2])),    -sin(float(xk_robot[2])),   0],
                       [sin(float(xk_robot[2])),            cos(float(xk_robot[2])),   0],
                       [0,                       0,                1.0]])
        J = np.array([[self.dt *self.wheel_radius/2  , self.dt *self.wheel_radius/2],
                            [0 , 0 ],
                            [self.dt *self.wheel_radius/self.wheel_base , -self.dt *self.wheel_radius/self.wheel_base]
                            ])
    ...
    def hf(self,xk ,H):
        ...
        for i in range(len(H)):           
            if(H[i] != 0):
                hf = np.block([[hf],[self.hfj(xk, H[i])]])
        return hf
