...
class PoseSLAMEKF:
    ...
    def OverlappingScan(self,xk ,max_num_scans, overlap_distancce ):
        '''
        ...
        dist, ind = tree.query(current_pose, k=max_num_scans)
        # Check if the distances are less than the overlap distance
    for i in range(len(distances)):
        if distances[i] < overlap_distance:
            H0.append(indices[i])
    ...
    def remove_pose(self, xk , Pk , len = 3):
        ...
        # print('________________________________________')
        # for l in range(len):
        #     xk = np.delete(xk, -3, axis=0)
        #     Pk = np.delete(Pk, -3, axis=0)
        #     Pk = np.delete(Pk, -3, axis=1)
