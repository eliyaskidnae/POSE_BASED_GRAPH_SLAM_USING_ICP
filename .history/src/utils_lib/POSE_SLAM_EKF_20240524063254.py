...
import threading
class PoseSLAMEKF:
    def __init__(self, x0, P0, Q,wheel_base , wheel_radius):    
        self.xk = x0
        self.Pk= P0
        self.Qk = Q
    ...
    def Add_New_Pose(self, xk, Pk):
        ...
        self.mutex.acquire()
        print("before", xk.shape , Pk.shape)
        ...
        print("after", xk_new.shape ,  self.Pk.shape)
    ...
    def Prediction(self, xk_1 , Pk_1 , uk , dt):             
        ...
        self.xk = np.block([[xk_1[:-3,:]], [xk_robot]])
        ...
        self.Pk = np.block([[A,C.T@Jfx.T],[Jfx@C,P]])
        # self.Pk =  Jfx@Pk_1@Jfx.T + Jfw@self.Qk@Jfw.T
        # self.Pk = F1k@ Pk_1 @ F1k.T + F2k@ self.Qk@F2k.T  
        # print("dete" , np.linalg.det(F1k@ Pk_1 @ F1k.T))
        t1 = rospy.Time.now().to_sec()
    ...
    def F2k(self,xk_robot):
        ...
        Jfw = np.array([[self.dt*cos(theta)/2,    self.dt*cos(theta)/2],
                        ...
                        [self.dt/self.wheel_base, -self.dt/self.wheel_base]]).reshape(3, 2)
     

        # F2k = np.block([[np.zeros((len(self.xk)-3, 2))],    
        #                 [Jfw]])
        return Jfw
    ...
    def hfj(self, xk, j ):
        ...
        # Wraap the angle between -pi and pi
        # hfj[-1] = self.wrap_angle(hfj[-1])

        # Kxn = Pose3D.ominus(NxBk)
        # hfk = Pose3D.oplus(Kxn , NxBj)
    ...
    def Jhf(self,xk,j):
        ...
        J = np.zeros((self.xb_dim,np.shape(self.xk)[0]))
    ...
    def jPk(self, xk, j):
        ...
        '''
        index = int(j*3)
        NPk = self.Pk[-3:,-3:]  # Extract the covariance of the robot pose
        NPj= self.Pk[index:index+3,index:index+3]  # Extract the covariance of the matched scan pose
        J1_oplus, J1_ominus, J2_oplus = self.Jhfjx(xk, j)
    ...
    def ObservationMatrix(self,xk,Hp ,zk ,Rk):
        ...
        """
        Hk, Vk = np.zeros((0,np.shape(self.xk)[0])), np.zeros((0,0))
        xk_robot = self.get_robot_pose(self.xk)
